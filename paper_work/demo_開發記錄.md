...
對這 20–40 台車，連線不是問題。`export_sumo.py` 已 `git checkout` 還原。
（那套理論後來在**另外 6 個路口**被量到是真的——§9.8——但不是這批車的原因。）
...
**第三次實機（09-07）**：兩格 `setRoute-failed 0`；路口內 deferred 2／8 台，幾步內全部重排完
（`still to retry 0`）；`gone 0`、`remove-failed 0`、沒有 `not known`。自測 PASS。

### 9.7 自測看不到、SUMO log 看得到的：車道盡頭的緊急煞停

同一次 log：

```
Vehicle 'e0_v8' performs emergency stop at the end of lane '1264707103_2403409706_0'
because there is no connection to the next edge ... time=250.00
```

封路那一秒改道的一台車。新路線在**邊**的層級合法（`setRoute` 只驗邊），但它在車道 0，
通往下一條邊的連線只從另一條車道出去，離路口不到幾十公尺、來不及變換車道，
於是在車道盡頭煞停。teleport 關著（§9.2），**它會永遠停在那裡把那條車道堵死**；
帳目上它還是 `driving`，自測看不出來。

| 修法 | 內容 |
|---|---|
| **離路口太近就緩改** | 改道前問每台車的車道、距車道盡頭幾公尺、該車道有沒有連到新路線的下一條邊（`lane.getLinks`，靜態、快取）。連不到且 **< 50 m** → 記為 deferred，路段變了再從實際到的那條邊重問 router——與路口內那套同一條路 |
| **重試改為「路段變了才重試」** | 原本每步重試，這種車會每步被拒一次 |
| **安全閥** | `--time-to-teleport.disconnected 60`：啟發式漏掉的車卡 60 秒後由 SUMO 挪到下一條邊，不永久堵路。每次計入 `teleported`，面板與自測都印，**自測要求為 0** |

**第四次實機（09-07）**：`teleported 0`、deferred 全部重排完、PASS。

### 9.8 GUI 才看得到的：直線弦，一個根、兩個症狀

自測 PASS 之後開 `DEMO_SUMO_GUI=1`，兩件自測看不到的事：

1. **畫面不對**（`demo/` 內有截圖）：整張網是端點間的直線——合併鏈的邊 CSV 只留兩端，
   netconvert 沒有 `shape` 就畫直線。4 km 的路一條線橫過市區、彎路變幾段折線、互相交叉。
   **和網頁在 `build_geometry.py` 之前一模一樣的問題**（§4），網頁修了、SUMO 沒有。
2. **封路後一行真的 `No connection`**：
   ```
   No connection between edge '5520441522_5521014889' and edge '5521014889_3285466348'
   ```
   這次沒有 permission 在作怪，就是字面意思。

兩件事同一個根。netconvert 用**邊抵達路口的方向**猜轉向；民權路那個路口，兩條合併鏈的
直線弦夾角 **176°**（真實路形只彎 30° 左右），而 A 沒有反向邊，於是 B 被當成 A 的迴轉、
不建連線。全圖直線弦夾角 ≥150° 的轉向有 27 個。

**不再猜 netconvert 的閾值，直接對帳**：拿用戶實際建出的 `.net.xml` 對照圖上每一個
u→v→w（w ≠ u；立即迴轉不會出現在任何 router 產出的路線上，Dijkstra 不繞圈、agent 不回訪）：

```
turns the graph allows : 2,767
present in the network : 2,761
missing                : 6
  13021803238_3285467668 -> 3285467668_13021803229   (民權路 -> 民權路)
  2286334583_5520565628 -> 5520565628_2286334586     (英才路 -> 英才路)
  2440630887_1264707098 -> 1264707098_2440630883     (文心路四段 -> 文心路四段)
  5094365563_13358756825 -> 13358756825_2249636453   (精武大橋 -> 精武大橋)
  5520441522_5521014889 -> 5521014889_3285466348     (民權路 -> 民權路)   ← GUI 拒的那個
  5520565628_2286334586 -> 2286334586_2286334583     (英才路 -> 英才路)
```

六個全是**急彎處的同一條路的延續**。§9.5 那套「netconvert 把折回去的弦當迴轉」的理論，
對這 6 個是對的；對那 20–40 台被拒的車是錯的。兩件都真，當時把第二件的證據算到第一件頭上。

**修法（`export_sumo.py`，09-07）**：

| | 做法 | 效果 |
|---|---|---|
| **`--shapes`** | 預設讀 `demo/arena_geometry.json`，投影後寫成每條邊的 `shape`（957 條有真實路形，其餘本來就直） | 畫面正確；netconvert 改用真實末段方向算夾角，≥150° 的轉向 27 → 14 |
| **`--check-net`** | 讀建好的 `.net.xml` 所有 `<connection>`，對照圖上 2,767 個轉向，缺的列出；`taichung.fix.con.xml` 為**每條有缺的進入邊寫出它全部的轉向**（逐車道） | 不猜閾值；有對帳結果 |
| **`build_net.sh` 兩段** | netconvert → 對帳 → 有缺才帶 `--connection-files` 重建 → 再對帳（`--strict`，仍缺就失敗） | 其餘 2,700 多個轉向維持 netconvert 自己的車道配置 |

【必讀】**`.con.xml` 的規則：進入邊只要出現在檔案裡，那份清單就是它的全部轉向。**
第一次跑兩段建網時 fix 檔只列了缺的那一對，第二段補了 3 個、**丟了另外 3 個**
（那三條進入邊原本自己建好的另一個出口，`Lane '5521014889_380038134_0' is not connected
from any incoming edge`）。改成每條有缺的進入邊列全部轉向：3 條邊、6 對、17 個車道對。

與 §9.5 被還原的那版差別：那版把 3,655 對邊**全部**逐車道明列、蓋掉 netconvert 的車道配置；
